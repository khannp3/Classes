...
void testConstructor1Invalid2_new() {
		// for constructor1
...
void testGetElementInvalid_new() {
	  // for constructor2
    int row=4,col=5;
    int data[4][5] = {{1,2,3,4,5},{6,7,8,9,0},{0,0,1,2,3},{0,0,0,4,5}};
    int*input_data[4];
	...
	string actual ="Empty for Now";
	try {
			int output = f.getElement(76, 2);	// invalid indexes
	}
	catch (std::out_of_range& e){
			 actual = e.what();
	}
...
void testCopy_new() {
	...
	int row=4,col=5;
    int data[4][5] = {{5,6,2,3,1},{6,7,8,9,0},{0,0,1,2,3},{0,0,0,4,5}};
	int*input_data[4];
	for (int i=0;i<4;i++){
	    	input_data[i]=data[i];
	}
...
void testAddToInvalid_new() {



	   /*Add-to Matrix*/
	    int row_to_add=40; int col_to_add=3;
...
void testSubMatrixInvalid_new() {

    int subinvalid_row = 5, subinvalid_col = 9;

	  int row=4,col=5;
      int data[4][5] = {{6,3,6,7,8},{1,6,3,4,6},{8,2,5,6,7},{0,0,0,4,5}};
	  int*input_data[4];
	  for (int i=0;i<4;i++){
	    	input_data[i]=data[i];
	  }
...
void testIsUpperTrFalseNotSquare_new() {

	int row=8,col=8;
    int upper1[8][8] = {{1,4,1},{1,9,1},{0,2,4}}; // not a square matrix
	int*input_upper1[3];
	for (int i=0;i<3;i++){
	    	input_upper1[i]=upper1[i];
	}
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testConstructor1Invalid2_new));
	s.push_back(CUTE(testGetElementInvalid_new));
	s.push_back(CUTE(testSetElementValid_new));
	s.push_back(CUTE(testAddToInvalid_new));
	s.push_back(CUTE(testSubMatrixInvalid_new));
	s.push_back(CUTE(testIsUpperTrFalseNotSquare_new));
